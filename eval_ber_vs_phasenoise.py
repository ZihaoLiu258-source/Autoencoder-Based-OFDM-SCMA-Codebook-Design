# ============================================================
# Evaluation: BER vs phase-noise sigma at fixed Eb/N0.
# Sweep axis: sigma_step. Parameter: CFO eps in {0, 0.04}.
# Output: SCMA_CFO_Simulation_Results.mat for MATLAB plotting.
# ============================================================
...
_MPL_CONFIG_DIR = Path(__file__).resolve().parent / "tmp/matplotlib"
...
# Parameter: only two CFO points are evaluated (0 and 0.04).
eps_vec = np.array([0.0, 0.04])
EPS_STEPS = len(eps_vec)

# Sweep axis: phase-noise sigma_step from 0 to 3e-3.
sigma_vec = np.linspace(0, 3e-3, 10)
...
def run():
    ...
    mat_filename = SWEEP_DIR / "SCMA_CFO_Simulation_Results.mat"
    sio.savemat(mat_filename, mat_data)
    print(f"[INFO] Saved {mat_filename}")

if __name__ == "__main__":
    SWEEP_DIR.mkdir(parents=True, exist_ok=True)
    run_with_log(run, str(SWEEP_DIR / "eval_ber_vs_phasenoise.log"))
