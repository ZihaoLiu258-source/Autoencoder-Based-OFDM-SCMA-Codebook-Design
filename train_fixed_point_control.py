...
The original ``train_ofdm_scma.py`` remains unchanged.  This wrapper resets the
requested seed, optionally disables only Soft-MED, changes into a dedicated
artifact directory, and then invokes the original training entry point.  The
generic checkpoint names are therefore confined to that directory and cannot
overwrite the paper's released seed-0 checkpoint in the repository root.
"""
