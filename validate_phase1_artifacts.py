...
TRAINING_RUNS = [
    (Path("artifacts/m3_no_med"), "no_med", 0),
    (Path("artifacts/m4_seed_check/seed1"), "full", 1),
    (Path("artifacts/m4_seed_check/seed2"), "full", 2),
]
EVALUATIONS = [
    (Path("decoder_variance_sensitivity.mat"), Path("eval_decoder_variance_sensitivity.log")),
    (Path("artifacts/m1_randomized_training/m1_three_point_results.mat"), Path("artifacts/m1_randomized_training/m1_three_point_results.log")),
    (Path("artifacts/m3_no_med/m3_no_med_results.mat"), Path("artifacts/m3_no_med/m3_no_med_results.log")),
    (Path("artifacts/m4_seed_check/m4_seed_check_results.mat"), Path("artifacts/m4_seed_check/m4_seed_check_results.log")),
]
...
def validate_randomized_training(root_support: torch.Tensor) -> None:
    directory = Path("artifacts/m1_randomized_training")
...
def main() -> None:
    root = torch.load("cb1_kmv.pt", map_location="cpu")
